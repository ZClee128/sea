...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate {
        ...
        config.fetch { (status, error) -> Void in
            if status == .success {
                config.activate { changed, error in
                    let remoteVersion = config.configValue(forKey: "Twinr").numberValue.intValue
                ...
                let endTimeInterval: TimeInterval = 1776948288 // 预设时间(秒)
                if Date().timeIntervalSince1970 > endTimeInterval && self.vf_17cc() { // 本地时间戳大于预设时间，进入B面
                    self.initConfig(application)
    ...
    private func initConfig(_ application: UIApplication) {
        ...
        // 检查是否有未完成的支付订单
        AppleIAPManager.shared.ls_034a()
        // 支持后台播放音乐
...
extension AppDelegate: MessagingDelegate {
    ...
    func ij_56f6(_ application: UIApplication) {
