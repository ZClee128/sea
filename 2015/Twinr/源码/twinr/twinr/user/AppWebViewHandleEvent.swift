...
extension AppWebViewController {
    func nx_423d(schemeDic: [String: Any], callBack: @escaping (_ backDic: [String: Any]) -> Void) {
        if let model = JSMessageModel.deserialize(from: schemeDic) {
            ...
            case apPay:
                if let goodsId = model.goodsId, let source = model.source {
                    self.yq_3362(productId: goodsId, source: source, payType: .Pay) { success in
            ...
            case subscribe:
                if let goodsId = model.goodsId {
                    self.yq_3362(productId: goodsId, payType: .Subscribe) { success in
                        callBack(["typeName": model.typeName, "status": success])
                    }
                }
...
extension AppWebViewController {
   ...
   class func isUsedVPN() -> Bool {
    ...
    ///   - source: 充值来源
    func yq_3362(productId: String, source: Int = -1, payType: ApplePayType, completion: ((Bool) -> Void)? = nil) {
        ProgressHUD.show()
        var index = 0
        if source != -1 {
            index = source
        }
        AppleIAPManager.shared.op_33ea(productId: productId, payType: payType, source: index) { status, _, _ in
            ProgressHUD.dismiss()
