...
extension AppWebViewController {
    func vb_3015(schemeDic: [String: Any], callBack: @escaping (_ backDic: [String: Any]) -> Void) {
        if let model = JSMessageModel.deserialize(from: schemeDic) {
            ...
            case apPay:
                if let goodsId = model.goodsId, let source = model.source {
                    self.ae_50ec(productId: goodsId, source: source, payType: .Pay) { success in
            ...
            case subscribe:
                if let goodsId = model.goodsId {
                    self.ae_50ec(productId: goodsId, payType: .Subscribe) { success in
                        callBack(["typeName": model.typeName, "status": success])
                    }
                }
...
extension AppWebViewController {
   ...
   class func isUsedVPN() -> Bool {
    ...
    ///   - source: 充值来源
    func ae_50ec(productId: String, source: Int = -1, payType: ApplePayType, completion: ((Bool) -> Void)? = nil) {
        ProgressHUD.show()
        var index = 0
        if source != -1 {
            index = source
        }
        AppleIAPManager.shared.sl_4134(productId: productId, payType: payType, source: index) { status, _, _ in
            ProgressHUD.dismiss()
