...
extension AppleIAPManager {
    ...
    ///   - productId: 产品Id
    ///   - block: 回调
    fileprivate func yg_7610(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
    ...
    ///   - transaction: 交易信息
    ///   - params: 接口参数
    fileprivate func fw_78d9(_ transactionId: String, params: [String: String]) {
        let reqModel = AppRequestModel.init()
        ...
        AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 2) {
                    self.zt_0895(transactionId, .Pay)
                }
                return
            }
...
extension AppleIAPManager {
    ...
    ///   - productId: 产品Id
    ///   - block: 回调
    fileprivate func ih_6f8c(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        let reqModel = AppRequestModel.init()
    ...
    ///   - transaction: 交易信息
    ///   - params: 接口参数
    fileprivate func co_6030(_ transactionId: String, params: [String: String]) {
        let reqModel = AppRequestModel.init()
        ...
        AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 3) {
                    self.zt_0895(transactionId, .Subscribe)
                }
                return
            }
...
extension AppleIAPManager {
    /// 初始化数据
    private func us_5553() {
        self.payCacheList = cq_10d2(payType: .Pay)
        self.subscribeCacheList = cq_10d2(payType: .Subscribe)
    ...
    /// - Returns: 缓存列表
    private func cq_10d2(payType: ApplePayType) -> [[String: String]] {
    ...
    ///   - payType: 支付类型
    /// - Returns: 收据数据
    fileprivate func wx_35aa(_ transactionId: String, _ payType: ApplePayType) -> String? {
        // 有未完成的订单，先取缓存
...
extension AppleIAPManager {
    ...
    func lw_6cc1() {
        ...
        for dict in self.payCacheList {
            fu_077d(dict["transactionId"], .Pay)
        }
        
        // 【订阅】失败重试
        for dict in self.subscribeCacheList {
            fu_077d(dict["transactionId"], .Subscribe)
        }
    }
    ...
    ///   - transactionId: Id
    ///   - payType: 支付类型
    private func fu_077d(_ transactionId: String?, _ payType: ApplePayType) {
        guard let transactionId = transactionId else { return }
        // 初始化每个交易请求次数
        reqRetryCountDict[transactionId] = 0
        // 3. 服务端校验流程
        zt_0895(transactionId, payType)
    }
}
...
extension AppleIAPManager {
    ...
    func on_3738(productId: String, payType: ApplePayType, source: Int = 0, handle: @escaping IAPcompletionHandle) {
        us_5553()
        ...
        switch(payType) {
        case .Pay:
            yg_7610(productId: productId, source: source) { [weak self] orderId, succeed in
        ...
        case .Subscribe:
            ih_6f8c(productId: productId, source: source) { [weak self] orderId, succeed in
                guard let self = self else { return }
                guard succeed == true && orderId != nil else { // 订单创建失败
...
extension AppleIAPManager: SKPaymentTransactionObserver {
        ...
        for transaction in transactions {
            ...
            case .purchased:   // 交易成功
                ...
                if transaction.original != nil && createOrderId == nil { // 启动自动续订时，不需要调用服务端验证接口
                    ...
                    // 3. 服务端校验流程
                    zt_0895(transaction.transactionIdentifier!, self.currentPayType)
                }
                // 移除苹果支付系统缓存
    ...
    ///   - payType: 支付类型
    fileprivate func zt_0895(_ transactionId: String, _ payType: ApplePayType) {
        guard let receiptStr = wx_35aa(transactionId, payType) else {
        ...
        case .Pay:
            ...
            fw_78d9(transactionId, params: paramsArr.first!)
            
        case .Subscribe:
            let paramsArr = self.subscribeCacheList.filter({$0["transactionId"] == transactionId})
            guard paramsArr.count > 0 else { return }
            co_6030(transactionId, params: paramsArr.first!)
        }
    }
}
