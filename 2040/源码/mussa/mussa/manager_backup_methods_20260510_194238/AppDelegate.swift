        ...
        config.fetch { (status, error) -> Void in
            if status == .success {
                config.activate { changed, error in
                    let remoteVersion = config.configValue(forKey: "Mussa").numberValue.intValue
                ...
                if Date().timeIntervalSince1970 > endTimeInterval && self.dr_0f00() { // 本地时间戳大于预设时间，进入B面
                    self.initConfig(application)
