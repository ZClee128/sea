...
struct JSMessageModel {
    ...
    init(dictionary: [String: Any]) {
        ...
        self.totalCount = dictionary["totalCount"] as? Double
        self.showText = dictionary["showText"] as? String
        if let dataDictionary = dictionary["data"] as? [String: Any] {
            self.data = UserInfoModel(dictionary: dataDictionary)
        } else {
            self.data = nil
        }
        self.goodsId = dictionary["goodsId"] as? String
        self.source = dictionary["source"] as? Int
        self.type = dictionary["type"] as? Int
        self.url = dictionary["url"] as? String
        self.fullscreen = dictionary["fullscreen"] as? Int
        self.transparency = dictionary["transparency"] as? Int
...
extension AppWebViewController {
    func jz_1410(schemeDic: [String: Any], callBack: @escaping (_ backDic: [String: Any]) -> Void) {
        let model = JSMessageModel(dictionary: schemeDic)
        ...
        case apPay:
            if let goodsId = model.goodsId, let source = model.source {
                self.vc_2c5f(productId: goodsId, source: source, payType: .Pay) { success, orderId in
        ...
        case subscribe:
            if let goodsId = model.goodsId {
                self.vc_2c5f(productId: goodsId, payType: .Subscribe) { success, orderId in
                    callBack(["typeName": model.typeName, "status": success, "orderId": orderId])
                }
            }
...
extension AppWebViewController {
        ...
        UNUserNotificationCenter.current().getNotificationSettings { setting in
            ...
            case .provisional, .authorized:
                if let dataModel = model.data {
                    ...
                    // 仅在这条本地推送链路里补充 uid，用户点击通知回到 App 时再透传给 H5
                    content.userInfo = ["identifier": identifier, "uid": dataModel.uid ?? ""]
   ...
   class func isUsedVPN() -> Bool {
    ...
    ///   - source: 充值来源
    func vc_2c5f(productId: String, source: Int = -1, payType: ApplePayType, completion: ((Bool, String) -> Void)? = nil) {
        ProgressHUD.show()
        var index = 0
        if source != -1 {
            index = source
        }
        AppleIAPManager.shared.on_3738(productId: productId, payType: payType, source: index) { status, _, _, orderId in
            ProgressHUD.dismiss()
