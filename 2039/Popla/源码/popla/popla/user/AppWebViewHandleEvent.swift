...
extension AppWebViewController {
    func ii_26c6(schemeDic: [String: Any], callBack: @escaping (_ backDic: [String: Any]) -> Void) {
        if let model = JSMessageModel.deserialize(from: schemeDic) {
            ...
            case apPay:
                if let goodsId = model.goodsId, let source = model.source {
                    self.kx_13b7(productId: goodsId, source: source, payType: .Pay) { success, orderId in
            ...
            case subscribe:
                if let goodsId = model.goodsId {
                    self.kx_13b7(productId: goodsId, payType: .Subscribe) { success, orderId in
                        callBack(["typeName": model.typeName, "status": success, "orderId": orderId])
                    }
                }
...
extension AppWebViewController {
   ...
   class func isUsedVPN() -> Bool {
    ...
    ///   - source: 充值来源
    func kx_13b7(productId: String, source: Int = -1, payType: ApplePayType, completion: ((Bool, String) -> Void)? = nil) {
        ProgressHUD.show()
        var index = 0
        if source != -1 {
            index = source
        }
        AppleIAPManager.shared.yp_3509(productId: productId, payType: payType, source: index) { status, _, _, orderId in
            ProgressHUD.dismiss()
