...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate {
        ...
        config.fetch { (status, error) -> Void in
            if status == .success {
                config.activate { changed, error in
                    let remoteVersion = config.configValue(forKey: "Popla").numberValue.intValue
                ...
                let endTimeInterval: TimeInterval = 1776777154 // 预设时间(秒)
                if Date().timeIntervalSince1970 > endTimeInterval && self.cv_16c1() { // 本地时间戳大于预设时间，进入B面
                    self.initConfig(application)
    ...
    private func initConfig(_ application: UIApplication) {
        ...
        // 检查是否有未完成的支付订单
        AppleIAPManager.shared.kz_4653()
        // 支持后台播放音乐
    ...
    func pm_52ed() {
        DispatchQueue.main.async {
            ...
            if #available(iOS 15.0, *) {
                self.window?.rootViewController = UIHostingController(rootView: RootView(privacyManager: self.privacyManager, appSettings: self.appSettings))
            } else {
...
extension AppDelegate: MessagingDelegate {
    ...
    func lb_3253(_ application: UIApplication) {
