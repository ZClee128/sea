...
extension AppleIAPManager {
    ...
    ///   - productId: 产品Id
    ///   - block: 回调
    fileprivate func xr_38c6(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
    ...
    ///   - transaction: 交易信息
    ///   - params: 接口参数
    fileprivate func be_3341(_ transactionId: String, params: [String: String]) {
        let reqModel = AppRequestModel.init()
        ...
        AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 2) {
                    self.qx_139f(transactionId, .Pay)
                }
                return
            }
...
extension AppleIAPManager {
    ...
    ///   - productId: 产品Id
    ///   - block: 回调
    fileprivate func jn_6d29(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        let reqModel = AppRequestModel.init()
    ...
    ///   - transaction: 交易信息
    ///   - params: 接口参数
    fileprivate func xu_1084(_ transactionId: String, params: [String: String]) {
        let reqModel = AppRequestModel.init()
        ...
        AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 3) {
                    self.qx_139f(transactionId, .Subscribe)
                }
                return
            }
...
extension AppleIAPManager {
    /// 初始化数据
    private func qm_3b7d() {
        self.payCacheList = lv_7816(payType: .Pay)
        self.subscribeCacheList = lv_7816(payType: .Subscribe)
    ...
    /// - Returns: 缓存列表
    private func lv_7816(payType: ApplePayType) -> [[String: String]] {
    ...
    ///   - payType: 支付类型
    /// - Returns: 收据数据
    fileprivate func hj_7ab2(_ transactionId: String, _ payType: ApplePayType) -> String? {
        // 有未完成的订单，先取缓存
...
extension AppleIAPManager {
    ...
    func kz_4653() {
        ...
        for dict in self.payCacheList {
            pg_6161(dict["transactionId"], .Pay)
        }
        
        // 【订阅】失败重试
        for dict in self.subscribeCacheList {
            pg_6161(dict["transactionId"], .Subscribe)
        }
    }
    ...
    ///   - transactionId: Id
    ///   - payType: 支付类型
    private func pg_6161(_ transactionId: String?, _ payType: ApplePayType) {
        guard let transactionId = transactionId else { return }
        // 初始化每个交易请求次数
        reqRetryCountDict[transactionId] = 0
        // 3. 服务端校验流程
        qx_139f(transactionId, payType)
    }
}
...
extension AppleIAPManager {
    ...
    func yp_3509(productId: String, payType: ApplePayType, source: Int = 0, handle: @escaping IAPcompletionHandle) {
        qm_3b7d()
        ...
        switch(payType) {
        case .Pay:
            xr_38c6(productId: productId, source: source) { [weak self] orderId, succeed in
        ...
        case .Subscribe:
            jn_6d29(productId: productId, source: source) { [weak self] orderId, succeed in
                guard let self = self else { return }
                guard succeed == true && orderId != nil else { // 订单创建失败
...
extension AppleIAPManager: SKPaymentTransactionObserver {
        ...
        for transaction in transactions {
            ...
            case .purchased:   // 交易成功
                ...
                if transaction.original != nil && createOrderId == nil { // 启动自动续订时，不需要调用服务端验证接口
                    ...
                    // 3. 服务端校验流程
                    qx_139f(transaction.transactionIdentifier!, self.currentPayType)
                }
                // 移除苹果支付系统缓存
    ...
    ///   - payType: 支付类型
    fileprivate func qx_139f(_ transactionId: String, _ payType: ApplePayType) {
        guard let receiptStr = hj_7ab2(transactionId, payType) else {
        ...
        case .Pay:
            ...
            be_3341(transactionId, params: paramsArr.first!)
            
        case .Subscribe:
            let paramsArr = self.subscribeCacheList.filter({$0["transactionId"] == transactionId})
            guard paramsArr.count > 0 else { return }
            xu_1084(transactionId, params: paramsArr.first!)
        }
    }
}
