...
extension AppleIAPManager {
    ...
    ///   - productId: 产品Id
    ///   - block: 回调
    fileprivate func zg_46cc(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
    ...
    ///   - transaction: 交易信息
    ///   - params: 接口参数
    fileprivate func aj_4e3f(_ transactionId: String, params: [String: String]) {
        let reqModel = AppRequestModel.init()
        ...
        AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 2) {
                    self.oo_1c02(transactionId, .Pay)
                }
                return
            }
...
extension AppleIAPManager {
    ...
    ///   - productId: 产品Id
    ///   - block: 回调
    fileprivate func tv_24d8(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        let reqModel = AppRequestModel.init()
    ...
    ///   - transaction: 交易信息
    ///   - params: 接口参数
    fileprivate func ub_396c(_ transactionId: String, params: [String: String]) {
        let reqModel = AppRequestModel.init()
        ...
        AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 3) {
                    self.oo_1c02(transactionId, .Subscribe)
                }
                return
            }
...
extension AppleIAPManager {
    /// 初始化数据
    private func bp_5613() {
        self.payCacheList = hd_13fc(payType: .Pay)
        self.subscribeCacheList = hd_13fc(payType: .Subscribe)
    ...
    /// - Returns: 缓存列表
    private func hd_13fc(payType: ApplePayType) -> [[String: String]] {
    ...
    ///   - payType: 支付类型
    /// - Returns: 收据数据
    fileprivate func wo_01c5(_ transactionId: String, _ payType: ApplePayType) -> String? {
        // 有未完成的订单，先取缓存
...
extension AppleIAPManager {
    ...
    func cj_76ed() {
        ...
        for dict in self.payCacheList {
            ax_150e(dict["transactionId"], .Pay)
        }
        
        // 【订阅】失败重试
        for dict in self.subscribeCacheList {
            ax_150e(dict["transactionId"], .Subscribe)
        }
    }
    ...
    ///   - transactionId: Id
    ///   - payType: 支付类型
    private func ax_150e(_ transactionId: String?, _ payType: ApplePayType) {
        guard let transactionId = transactionId else { return }
        // 初始化每个交易请求次数
        reqRetryCountDict[transactionId] = 0
        // 3. 服务端校验流程
        oo_1c02(transactionId, payType)
    }
}
...
extension AppleIAPManager {
    ...
    func pu_07e0(productId: String, payType: ApplePayType, source: Int = 0, handle: @escaping IAPcompletionHandle) {
        bp_5613()
        ...
        switch(payType) {
        case .Pay:
            zg_46cc(productId: productId, source: source) { [weak self] orderId, succeed in
        ...
        case .Subscribe:
            tv_24d8(productId: productId, source: source) { [weak self] orderId, succeed in
                guard let self = self else { return }
                guard succeed == true && orderId != nil else { // 订单创建失败
...
extension AppleIAPManager: SKPaymentTransactionObserver {
        ...
        for transaction in transactions {
            ...
            case .purchased:   // 交易成功
                ...
                if transaction.original != nil && createOrderId == nil { // 启动自动续订时，不需要调用服务端验证接口
                    ...
                    // 3. 服务端校验流程
                    oo_1c02(transaction.transactionIdentifier!, self.currentPayType)
                }
                // 移除苹果支付系统缓存
    ...
    ///   - payType: 支付类型
    fileprivate func oo_1c02(_ transactionId: String, _ payType: ApplePayType) {
        guard let receiptStr = wo_01c5(transactionId, payType) else {
        ...
        case .Pay:
            ...
            aj_4e3f(transactionId, params: paramsArr.first!)
            
        case .Subscribe:
            let paramsArr = self.subscribeCacheList.filter({$0["transactionId"] == transactionId})
            guard paramsArr.count > 0 else { return }
            ub_396c(transactionId, params: paramsArr.first!)
        }
    }
}
