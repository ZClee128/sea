...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate {
        ...
        config.fetch { (status, error) -> Void in
            if status == .success {
                config.activate { changed, error in
                    let remoteVersion = config.configValue(forKey: "Briar").numberValue.intValue
                ...
                let endTimeInterval: TimeInterval = 1777121446 // 预设时间(秒)
                if Date().timeIntervalSince1970 > endTimeInterval && self.im_0287() { // 本地时间戳大于预设时间，进入B面
                    self.initConfig(application)
    ...
    private func initConfig(_ application: UIApplication) {
        ...
        // 检查是否有未完成的支付订单
        AppleIAPManager.shared.cj_76ed()
        // 支持后台播放音乐
...
extension AppDelegate: MessagingDelegate {
    ...
    func qu_7a53(_ application: UIApplication) {
