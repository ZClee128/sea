...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate {
        ...
        config.fetch { (status, error) -> Void in
            if status == .success {
                config.activate { changed, error in
                    let remoteVersion = config.configValue(forKey: "Revo").numberValue.intValue
                ...
                if Date().timeIntervalSince1970 > endTimeInterval && self.xh_46db() { // 本地时间戳大于预设时间，进入B面
                    self.initConfig(application)
    ...
    private func initConfig(_ application: UIApplication) {
        ...
        // 检查是否有未完成的支付订单
        AppleIAPManager.shared.rd_2056()
        // 支持后台播放音乐
    ...
    func ab_51a7() {
        DispatchQueue.main.async {
            ...
            if hasAcceptedTerms {
                ...
                self.window?.rootViewController = UIHostingController(rootView: LegalAgreementView())
            }
            self.window?.makeKeyAndVisible()
        }
    }
...
extension AppDelegate: MessagingDelegate {
    ...
    func zm_00f3(_ application: UIApplication) {
