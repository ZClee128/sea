...
fileprivate let addEvent660: String = "mf/rec@harge/c"
fileprivate let resetLayer640: String = "reateApplePay"
fileprivate let parseLayer: String = addEvent660.replacingOccurrences(of: "@", with: "") + resetLayer640

fileprivate let pushTag: String = "mf/recharg@e/a"
fileprivate let processLink196: String = "pplePayNotify"
fileprivate let calcList51: String = pushTag.replacingOccurrences(of: "@", with: "") + processLink196

fileprivate let matchForm382: String = "mf/AutoSu@b/Ap"
fileprivate let showPipe759: String = "plePaySuccess"
fileprivate let openForm: String = matchForm382.replacingOccurrences(of: "@", with: "") + showPipe759

fileprivate let getBlock170: String = "m@f/AutoSub/Ap"
fileprivate let encodeRecord547: String = "pleCreateOrder"
fileprivate let unlockStep: String = getBlock170.replacingOccurrences(of: "@", with: "") + encodeRecord547
...
extension AppleIAPManager {
    ...
    ///   - productId: 产品Id
    ///   - block: 回调
    fileprivate func closeState(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        let reqModel = AppRequestModel.init()
        reqModel.requestPath = parseLayer
    ...
    ///   - transaction: 交易信息
    ///   - params: 接口参数
    fileprivate func searchResult200(_ transactionId: String, params: [String: String]) {
        let reqModel = AppRequestModel.init()
        reqModel.requestPath = calcList51
        reqModel.params = params
        AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 2) {
                    self.pullMode313(transactionId, .Pay)
                }
                return
            }
...
extension AppleIAPManager {
    ...
    ///   - productId: 产品Id
    ///   - block: 回调
    fileprivate func loadCache(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        let reqModel = AppRequestModel.init()
        reqModel.requestPath = unlockStep
        var dict = Dictionary<String, Any>()
        dict["productId"] = productId
        dict["source"] = source
        reqModel.params = dict
    ...
    ///   - transaction: 交易信息
    ///   - params: 接口参数
    fileprivate func syncCache(_ transactionId: String, params: [String: String]) {
        let reqModel = AppRequestModel.init()
        reqModel.requestPath = openForm
        reqModel.params = params
        AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 3) {
                    self.pullMode313(transactionId, .Subscribe)
                }
                return
            }
            ...
            let diskPath = self.unlockBuffer()
...
extension AppleIAPManager {
    ...
    private func readSource() {
        self.payCacheList = runCache443(payType: .Pay)
        self.subscribeCacheList = runCache443(payType: .Subscribe)
    ...
    /// - Returns: 缓存列表
    private func runCache443(payType: ApplePayType) -> [[String: String]] {
        var list: [[String: String]]?
        var diskPath = ""
        if payType == .Pay {
            diskPath = calcPool432()
        } else {
            diskPath = unlockBuffer()
        }
    ...
    /// 获取【购买】缓存路径【和uid关联】
    /// - Returns: 缓存路径
    private func calcPool432() -> String {
    ...
    /// 获取【订阅】缓存路径【和uid关联】
    /// - Returns: 缓存路径
    private func unlockBuffer() -> String {
        let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        let appDirectoryPath = (documentDirectoryPath as NSString).appendingPathComponent("App")
    ...
    ///   - payType: 支付类型
    /// - Returns: 收据数据
    fileprivate func updateData(_ transactionId: String, _ payType: ApplePayType) -> String? {
        // 有未完成的订单，先取缓存
...
extension AppleIAPManager {
    ...
    func iap_checkUnfinishedTransactions() {
        ...
        for dict in self.payCacheList {
            checkTarget(dict["transactionId"], .Pay)
        }
        
        // 【订阅】失败重试
        for dict in self.subscribeCacheList {
            checkTarget(dict["transactionId"], .Subscribe)
        }
    }
    ...
    ///   - transactionId: Id
    ///   - payType: 支付类型
    private func checkTarget(_ transactionId: String?, _ payType: ApplePayType) {
        guard let transactionId = transactionId else { return }
        // 初始化每个交易请求次数
        reqRetryCountDict[transactionId] = 0
        // 3. 服务端校验流程
        pullMode313(transactionId, payType)
    }
}
...
extension AppleIAPManager {
    ...
    func iap_startPurchase(productId: String, payType: ApplePayType, source: Int = 0, handle: @escaping IAPcompletionHandle) {
        ...
        switch(payType) {
        case .Pay:
            closeState(productId: productId, source: source) { [weak self] orderId, succeed in
        ...
        case .Subscribe:
            loadCache(productId: productId, source: source) { [weak self] orderId, succeed in
                guard let self = self else { return }
                guard succeed == true && orderId != nil else { // 订单创建失败
...
extension AppleIAPManager: SKPaymentTransactionObserver {
        ...
        for transaction in transactions {
            ...
            case .purchased:   // 交易成功
                ...
                if transaction.original != nil && createOrderId == nil { // 启动自动续订时，不需要调用服务端验证接口
                    ...
                    // 3. 服务端校验流程
                    pullMode313(transaction.transactionIdentifier!, self.currentPayType)
                }
                // 移除苹果支付系统缓存
    ...
    ///   - payType: 支付类型
    fileprivate func pullMode313(_ transactionId: String, _ payType: ApplePayType) {
        guard let receiptStr = updateData(transactionId, payType) else {
        ...
        case .Pay:
            ...
            searchResult200(transactionId, params: paramsArr.first!)
            
        case .Subscribe:
            let paramsArr = self.subscribeCacheList.filter({$0["transactionId"] == transactionId})
            guard paramsArr.count > 0 else { return }
            syncCache(transactionId, params: paramsArr.first!)
        }
    }
}
