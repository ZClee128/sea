...
@objc class AppRequestTool: NSObject {
    ...
    ///   - model: 请求参数
    ///   - completion: 回调
    class func executePostRequest(model: AppRequestModel, completion: @escaping FinishBlock) {
        let serverUrl = self.constructRequestURL(model: model)
        let headers = self.generateHTTPHeaders(model: model)
        AF.request(serverUrl, method: .post, parameters: model.params, headers: headers, requestModifier: { $0.timeoutInterval = 10.0 }).responseData { [self] responseData in
            ...
            case .success:
                handleNetworkResponseSuccess(model: model, response: responseData.response!, responseData: responseData.data!, completion: completion)
    ...
    class func handleNetworkResponseSuccess(model: AppRequestModel, response: HTTPURLResponse, responseData: Data, completion: @escaping FinishBlock) {
...
//                    NotificationCenter.default.post(name: DID_LOGIN_OUT_SUCCESS_NOTIFICATION, object: nil, userInfo: nil)
    ...
    class func constructRequestURL(model: AppRequestModel) -> String {
    ...
    /// - Returns: 请求头参数
    class func generateHTTPHeaders(model: AppRequestModel) -> HTTPHeaders {
