...
class AppAdjustManager: NSObject {
    ...
    func initAdjust() {
        ...
        let adjustConfig = ADJConfig(appToken: _hiesem, environment: environment)
        adjustConfig?.logLevel = ADJLogLevelWarn
        adjustConfig?.delegate = self
        Adjust.appDidLaunch(adjustConfig)
        AppAdjustManager.addOnceEvent(token: _lnh9k3o9j755)
    }
}
...
extension AppAdjustManager: AdjustDelegate {
    ...
    private class func bindExternalId(uid: String) {
        let hashed = sha256(uid)
        Adjust.addSessionPartnerParameter("external_id", value: hashed)
    }

    /// SHA-256 哈希，输出小写十六进制字符串（与服务端保持一致）
    private class func sha256(_ input: String) -> String {
