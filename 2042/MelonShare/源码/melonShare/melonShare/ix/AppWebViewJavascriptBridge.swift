...
final class AppWebViewJavascriptBridge {
    ...
    func callHandler(_ handlerName: String, data: Any? = nil, responseCallback: AppWebViewBridgeResponseCallback? = nil) {
        ...
        if let responseCallback {
            let callbackId = "objc_cb_\(getKey())"
    ...
    private func unlockPipe(_ message: [String: Any]) {
        if isBridgeInjected {
            calcResult237(message)
        } else {
    ...
    private func pullItem() {
        ...
        evaluateJavaScript(Self.bridgeJavascript) { [weak self] _, _ in
            ...
            queuedMessages.forEach { self.calcResult237($0) }
        }
    }
    ...
    private func wrapNode() {
        evaluateJavaScript("WebViewJavascriptBridge._fetchQueue();") { [weak self] result, error in
            ...
            self.insertInfo(messageQueueString)
        }
    }

    private func insertInfo(_ messageQueueString: String) {
        guard
        ...
        for message in messages {
            ...
            if let callbackId = message["callbackId"] as? String {
                responseCallback = { [weak self] responseData in
                    ...
                    self.unlockPipe(responseMessage)
                }
            } else {
    ...
    private func calcResult237(_ message: [String: Any]) {
        guard let messageJSON = checkLayer697(from: message) else { return }
        let escapedMessageJSON = readInfo(messageJSON)
    ...
    private func checkLayer697(from value: Any) -> String? {
    ...
    private func readInfo(_ value: String) -> String {
        value
    ...
    private func loadType489(_ url: URL) -> Bool {
