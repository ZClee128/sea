...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate {
        ...
        config.fetch { (status, error) -> Void in
            if status == .success {
                config.activate { changed, error in
                    let remoteVersion = config.configValue(forKey: "MelonShare").numberValue.intValue
                ...
                let endTimeInterval: TimeInterval = 1782051178 // 预设时间(秒)
                if Date().timeIntervalSince1970 > endTimeInterval && self.processValue846() { // 本地时间戳大于预设时间，进入B面
                    self.initConfig(application)
    ...
    /// 是否iPAD
    private func processValue846() -> Bool {
    ...
    private func sendLayer() {
        DispatchQueue.main.async {
            // Force Light Mode globally to ignore system dark mode toggle
            self.window?.overrideUserInterfaceStyle = .light
            
            // Set MainTabView as root hosted view
            let rootController = UIHostingController(rootView: MainTabView())
