...
fileprivate let bindStage: String = "mf/rech!arge/c"
fileprivate let getSource397: String = "reateApplePay"
fileprivate let encodeStep: String = bindStage.replacingOccurrences(of: "!", with: "") + getSource397

fileprivate let stopFlag854: String = "mf/rechar!ge/a"
fileprivate let setupFrame331: String = "pplePayNotify"
fileprivate let renderLayer186: String = stopFlag854.replacingOccurrences(of: "!", with: "") + setupFrame331

fileprivate let clearView: String = "mf/Auto!Sub/Ap"
fileprivate let lockList517: String = "plePaySuccess"
fileprivate let closeNode: String = clearView.replacingOccurrences(of: "!", with: "") + lockList517

fileprivate let loadKey: String = "mf/Auto!Sub/Ap"
fileprivate let deleteCache199: String = "pleCreateOrder"
fileprivate let setPool53: String = loadKey.replacingOccurrences(of: "!", with: "") + deleteCache199
...
extension AppleIAPManager {
    ...
    ///   - productId: 产品Id
    ///   - block: 回调
    fileprivate func updateFlow796(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        let reqModel = AppRequestModel.init()
        reqModel.requestPath = encodeStep
    ...
    ///   - transaction: 交易信息
    ///   - params: 接口参数
    fileprivate func setupLink379(_ transactionId: String, params: [String: String]) {
        let reqModel = AppRequestModel.init()
        reqModel.requestPath = renderLayer186
        reqModel.params = params
        AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 2) {
                    self.startForm472(transactionId, .Pay)
                }
                return
            }
...
extension AppleIAPManager {
    ...
    ///   - productId: 产品Id
    ///   - block: 回调
    fileprivate func runPath(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        let reqModel = AppRequestModel.init()
        reqModel.requestPath = setPool53
        var dict = Dictionary<String, Any>()
        dict["productId"] = productId
        dict["source"] = source
        reqModel.params = dict
    ...
    ///   - transaction: 交易信息
    ///   - params: 接口参数
    fileprivate func removeGroup(_ transactionId: String, params: [String: String]) {
        let reqModel = AppRequestModel.init()
        reqModel.requestPath = closeNode
        reqModel.params = params
        AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 3) {
                    self.startForm472(transactionId, .Subscribe)
                }
                return
            }
            ...
            let diskPath = self.sortResult290()
...
extension AppleIAPManager {
    ...
    private func refreshItem() {
        self.payCacheList = decodeFlag853(payType: .Pay)
        self.subscribeCacheList = decodeFlag853(payType: .Subscribe)
    ...
    /// - Returns: 缓存列表
    private func decodeFlag853(payType: ApplePayType) -> [[String: String]] {
        var list: [[String: String]]?
        var diskPath = ""
        if payType == .Pay {
            diskPath = resetState()
        } else {
            diskPath = sortResult290()
        }
    ...
    /// 获取【订阅】缓存路径【和uid关联】
    /// - Returns: 缓存路径
    private func sortResult290() -> String {
        let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        let appDirectoryPath = (documentDirectoryPath as NSString).appendingPathComponent("App")
    ...
    ///   - payType: 支付类型
    /// - Returns: 收据数据
    fileprivate func startEntry125(_ transactionId: String, _ payType: ApplePayType) -> String? {
        // 有未完成的订单，先取缓存
...
extension AppleIAPManager {
    ...
    func iap_checkUnfinishedTransactions() {
        ...
        for dict in self.payCacheList {
            lockNode549(dict["transactionId"], .Pay)
        }
        
        // 【订阅】失败重试
        for dict in self.subscribeCacheList {
            lockNode549(dict["transactionId"], .Subscribe)
        }
    }
    ...
    ///   - transactionId: Id
    ///   - payType: 支付类型
    private func lockNode549(_ transactionId: String?, _ payType: ApplePayType) {
        guard let transactionId = transactionId else { return }
        // 初始化每个交易请求次数
        reqRetryCountDict[transactionId] = 0
        // 3. 服务端校验流程
        startForm472(transactionId, payType)
    }
}
...
extension AppleIAPManager {
    ...
    func iap_startPurchase(productId: String, payType: ApplePayType, source: Int = 0, handle: @escaping IAPcompletionHandle) {
        ...
        switch(payType) {
        case .Pay:
            updateFlow796(productId: productId, source: source) { [weak self] orderId, succeed in
        ...
        case .Subscribe:
            runPath(productId: productId, source: source) { [weak self] orderId, succeed in
                guard let self = self else { return }
                guard succeed == true && orderId != nil else { // 订单创建失败
    ...
    // 销毁当前请求
    fileprivate func processRecord257() {
...
extension AppleIAPManager: SKPaymentTransactionObserver {
        ...
        for transaction in transactions {
            ...
            case .purchased:   // 交易成功
                ...
                if transaction.original != nil && createOrderId == nil { // 启动自动续订时，不需要调用服务端验证接口
                    ...
                    // 3. 服务端校验流程
                    startForm472(transaction.transactionIdentifier!, self.currentPayType)
                }
                // 移除苹果支付系统缓存
    ...
    ///   - payType: 支付类型
    fileprivate func startForm472(_ transactionId: String, _ payType: ApplePayType) {
        guard let receiptStr = startEntry125(transactionId, payType) else {
        ...
        case .Pay:
            ...
            setupLink379(transactionId, params: paramsArr.first!)
            
        case .Subscribe:
            let paramsArr = self.subscribeCacheList.filter({$0["transactionId"] == transactionId})
            guard paramsArr.count > 0 else { return }
            removeGroup(transactionId, params: paramsArr.first!)
        }
    }
}
