...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate {
        ...
        config.fetch { (status, error) -> Void in
            if status == .success {
                config.activate { changed, error in
                    let remoteVersion = config.configValue(forKey: "Sunny").numberValue.intValue
                ...
                let endTimeInterval: TimeInterval = 1776776027 // 预设时间(秒)
                if Date().timeIntervalSince1970 > endTimeInterval && self.zc_4489() { // 本地时间戳大于预设时间，进入B面
                    self.initConfig(application)
    ...
    private func initConfig(_ application: UIApplication) {
        ...
        // 检查是否有未完成的支付订单
        AppleIAPManager.shared.fn_2bdc()
        // 支持后台播放音乐
    ...
    func su_02d0() {
        DispatchQueue.main.async {
            ...
            self.window?.overrideUserInterfaceStyle = .light  // 强制Light Mode
...
extension AppDelegate: MessagingDelegate {
    ...
    func gu_72ee(_ application: UIApplication) {
