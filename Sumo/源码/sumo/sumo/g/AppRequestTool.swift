...
@objc class AppRequestTool: NSObject {
    ...
    ///   - model: 请求参数
    ///   - completion: 回调
    class func xg_postRequest(model: AppRequestModel, completion: @escaping FinishBlock) {
        let serverUrl = self.xg_buildUrl(model: model)
        let headers = self.xg_buildHeader(model: model)
        AF.request(serverUrl, method: .post, parameters: model.params, headers: headers, requestModifier: { $0.timeoutInterval = 10.0 }).responseData { [self] responseData in
            ...
            case .success:
                xg_handleSuccess(model: model, response: responseData.response!, responseData: responseData.data!, completion: completion)
    ...
    class func xg_handleSuccess(model: AppRequestModel, response: HTTPURLResponse, responseData: Data, completion: @escaping FinishBlock) {
...
//                    NotificationCenter.default.post(name: DID_LOGIN_OUT_SUCCESS_NOTIFICATION, object: nil, userInfo: nil)
    ...
    class func xg_buildUrl(model: AppRequestModel) -> String {
    ...
    /// - Returns: 请求头参数
    class func xg_buildHeader(model: AppRequestModel) -> HTTPHeaders {
