...
class AppAdjustManager: NSObject {
    ...
    /// 初始化Adjust
    func configureTrackingServices() {
        ...
        AppAdjustManager.logUniqueTransaction(token: AdInstallToken)
    }
}
...
extension AppAdjustManager: AdjustDelegate {
    /// 获取设备id
    class func fetchDeviceIdentifier() -> String {
    ...
    /// 添加去重事件【只记录一次】
    /// - Parameter key: 事件名
    class func logUniqueTransaction(token: String) {
    ...
    ///   - token: token
    ///   - count: 价格
    class func trackRevenueTransaction(token: String, count: Double) {
        let event = ADJEvent(eventToken: token)
    ...
    /// 添加埋点事件
    /// - Parameter key: 事件名
    class func trackCustomEvent(token: String) {
        let event = ADJEvent(eventToken: token)
        Adjust.trackEvent(event)
    }
}
