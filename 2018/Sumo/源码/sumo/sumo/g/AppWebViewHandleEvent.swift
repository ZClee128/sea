...
extension AppWebViewController {
    func xg_handleJSMessage(schemeDic: [String: Any], callBack: @escaping (_ backDic: [String: Any]) -> Void) {
        if let model = JSMessageModel.deserialize(from: schemeDic) {
            switch model.typeName {
            case getDeviceID:
                let adidStr = AppAdjustManager.xg_fetchAdid()
                callBack(["typeName": model.typeName, "deviceID": adidStr])

            case getFirebaseID:
                AppWebViewController.xg_fetchFBToken { str in
                    callBack(["typeName": model.typeName, "fireBaseID": str])
                }
                
            case getAreaISO:
                let arr = AppWebViewController.xg_fetchCountryISO()
                callBack(["typeName": model.typeName, "areaISO": arr.joined(separator: ",")])
                
            case getProxyStatus:
                let status = AppWebViewController.xg_checkProxyStatus()
            ...
            case inAppRating:
                ...
                AppWebViewController.xg_requestRating()

            case apPay:
                if let goodsId = model.goodsId, let source = model.source {
                    self.xg_processApplePay(productId: goodsId, source: source, payType: .Pay) { success in
            ...
            case subscribe:
                if let goodsId = model.goodsId {
                    self.xg_processApplePay(productId: goodsId, payType: .Subscribe) { success in
                        callBack(["typeName": model.typeName, "status": success])
                    }
                }
            ...
            case openNewWebview:
                ...
                if let urlStr = model.url,
                    ...
                    AppWebViewController.xg_openWebView(urlStr, transparency, fullscreen)
                }
            ...
            case getNotificationStatus:
                AppPermissionTool.shared.xg_reqNotifPerm { auth, isFirst in
                    callBack(["typeName": model.typeName, "isAuth": auth, "isFirst": isFirst])
                }
            
            case getMicStatus:
                AppPermissionTool.shared.xg_reqMicPerm { auth, isFirst in
                    callBack(["typeName": model.typeName, "isAuth": auth, "isFirst": isFirst])
                }
                
            case getPhotoStatus:
                AppPermissionTool.shared.xg_reqPhotoPerm { auth, isFirst in
                    callBack(["typeName": model.typeName, "isAuth": auth, "isFirst": isFirst])
                }
                
            case getCameraStatus:
                AppPermissionTool.shared.xg_reqCameraPerm { auth, isFirst in
                    callBack(["typeName": model.typeName, "isAuth": auth, "isFirst": isFirst])
                }
            ...
            case reportAdjust:
                if let token = model.token {
                    if let count = model.totalCount {
                        AppAdjustManager.xg_logPurchase(token: token, count: count)
                    } else {
                        AppAdjustManager.xg_logEvent(token: token)
                    }
                }
                callBack(["typeName": model.typeName])
            ...
            case requestLocalPush:
                callBack(["typeName": model.typeName])
                AppWebViewController.xg_pushLocalNotif(model)
...
extension AppWebViewController {
    ...
    class func xg_openWebView(_ urlStr: String, _ transparency: Int = 0, _ fullscreen: Int = 1) {
        let vc = AppWebViewController()
        vc.urlString = urlStr
        vc.clearBgColor = (transparency == 1)
        vc.fullscreen = (fullscreen == 1)
        vc.modalPresentationStyle = .fullScreen
        AppConfig.xg_topViewController()?.present(vc, animated: true)
    }
    
    /// 本地推送
    class func xg_pushLocalNotif(_ model: JSMessageModel) {
    ...
    /// 获取firebase token
    class func xg_fetchFBToken(tokenBlock: @escaping (_ str: String) -> Void) {
    ...
    /// 获取ISO国家代码
    class func xg_fetchCountryISO() -> [String] {
        var tempArr: [String] = []
    ...
    /// 获取用户代理状态
    class func xg_checkProxyStatus() -> Bool {
        if AppWebViewController.xg_hasProxy() || AppWebViewController.xg_hasVPN() {
            return true
        }
        return false
    }
   ...
   class func xg_hasVPN() -> Bool {
    ...
    ///   - source: 充值来源
    func xg_processApplePay(productId: String, source: Int = -1, payType: ApplePayType, completion: ((Bool) -> Void)? = nil) {
        ProgressHUD.show()
        var index = 0
        if source != -1 {
            index = source
        }
        AppleIAPManager.shared.xg_startPurchase(productId: productId, payType: payType, source: index) { status, _, _ in
            ProgressHUD.dismiss()
