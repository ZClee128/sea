...
/// 域名
let ReplaceUrlDomain = ["co", "de", "ga", "lx"].joined()
/// 包ID
let PackageID = "2017"
/// Adjust
let AdjustKey = "vznnugiv0qo0"
let AdInstallToken = "6ea9ux"
...
class AppConfig: NSObject {
    /// 获取状态栏高度
    class func fetchSbHVal() -> CGFloat {
    ...
    /// 获取当前控制器
    class func getTopVCCtrl() -> (UIViewController?) {
        var window = AppConfig.reqWinObj()
        if window.windowLevel != UIWindow.Level.normal {
            let windows = UIApplication.shared.windows
            for windowTemp in windows {
                if windowTemp.windowLevel == UIWindow.Level.normal {
        ...
        return getTopVCCtrl(vc)
    }

    class func getTopVCCtrl(_ vc: UIViewController?)
        -> UIViewController?
    {
        if vc == nil {
            return nil
        ...
        if let presentVC = vc?.presentedViewController {
            return getTopVCCtrl(presentVC)
        } else if let tabVC = vc as? UITabBarController {
            if let selectVC = tabVC.selectedViewController {
                return getTopVCCtrl(selectVC)
            }
            return nil
        } else if let naiVC = vc as? UINavigationController {
            return getTopVCCtrl(naiVC.visibleViewController)
        } else {
            return vc
        }
    }
