...
extension AppWebViewController {
    func handleH5Message(schemeDic: [String: Any], callBack: @escaping (_ backDic: [String: Any]) -> Void) {
        if let model = JSMessageModel.deserialize(from: schemeDic) {
            ...
            case getDeviceID:
                let adidStr = AppAdjustManager.fetchAdjId()
                callBack(["typeName": model.typeName, "deviceID": adidStr])

            case getFirebaseID:
                AppWebViewController.reqFbTkn { str in
            ...
            case openNewWebview:
                ...
                if let urlStr = model.url,
                    ...
                    AppWebViewController.launchNwv(urlStr, transparency, fullscreen)
                }
            ...
            case reportAdjust:
                if let token = model.token {
                    if let count = model.totalCount, count > 0 {
                        AppAdjustManager.recordPurEvt(tkn: token, cnt: count)
                    } else {
                        AppAdjustManager.recordEvt(tkn: token)
                    }
                }
                callBack(["typeName": model.typeName])
            ...
            case requestLocalPush:
                callBack(["typeName": model.typeName])
                AppWebViewController.trigLocNoti(model)
...
extension AppWebViewController {
    ...
    class func launchNwv(_ urlStr: String, _ transparency: Int = 0, _ fullscreen: Int = 1) {
        let vc = AppWebViewController()
        vc.urlString = urlStr
        vc.clearBgColor = (transparency == 1)
        vc.fullscreen = (fullscreen == 1)
        vc.modalPresentationStyle = .fullScreen
        AppConfig.getTopVCCtrl()?.present(vc, animated: true)
    }
    
    /// 本地推送
    class func trigLocNoti(_ model: JSMessageModel) {
    ...
    /// 获取firebase token
    class func reqFbTkn(tokenBlock: @escaping (_ str: String) -> Void) {
    ...
    class func getUsedProxyStatus() -> Bool {
        if AppWebViewController.chkPxyUse() || AppWebViewController.chkVpnUse() {
            return true
        }
        return false
    }
   ...
   class func chkVpnUse() -> Bool {
    ...
    func applePay(productId: String, source: Int = -1, payType: ApplePayType, completion: ((Bool) -> Void)? = nil) {
        ...
        AppleIAPManager.shared.beginIapPur(productId: productId, payType: payType, source: index) { status, _, _ in
            ProgressHUD.dismiss()
