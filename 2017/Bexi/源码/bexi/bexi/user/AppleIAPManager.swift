...
extension AppleIAPManager {
    ...
    fileprivate func req_pay_createAppleOrder(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        ...
        AppRequestTool.initPostReq(model: reqModel) { succeed, result, errorModel in
        ...
        AppRequestTool.initPostReq(model: reqModel) { succeed, result, errorModel in
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 2) {
                    self.chkTxPur(transactionId, .Pay)
                }
                return
            }
            ...
            let diskPath = self.reqPayCchPth()
        ...
        AppRequestTool.initPostReq(model: reqModel) { succeed, result, errorModel in
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 3) {
                    self.chkTxPur(transactionId, .Subscribe)
                }
                return
            }
            ...
            let diskPath = self.reqSubCchPth()
...
extension AppleIAPManager {
    ...
    private func getLocalPayCacheList(payType: ApplePayType) -> [[String: String]] {
        ...
        var diskPath = ""
        if payType == .Pay {
            diskPath = reqPayCchPth()
        } else {
            diskPath = reqSubCchPth()
        }
    ...
    /// 获取【购买】缓存路径【和uid关联】
    /// - Returns: 缓存路径
    private func reqPayCchPth() -> String {
    ...
    /// 获取【订阅】缓存路径【和uid关联】
    /// - Returns: 缓存路径
    private func reqSubCchPth() -> String {
        let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        let appDirectoryPath = (documentDirectoryPath as NSString).appendingPathComponent("App")
    ...
    ///   - payType: 支付类型
    /// - Returns: 收据数据
    fileprivate func reqVfyDt(_ transactionId: String, _ payType: ApplePayType) -> String? {
        // 有未完成的订单，先取缓存
...
extension AppleIAPManager {
    ...
    private func iap_failedRetry(_ transactionId: String?, _ payType: ApplePayType) {
        ...
        // 3. 服务端校验流程
        chkTxPur(transactionId, payType)
    }
}
...
extension AppleIAPManager {
    ...
    func beginIapPur(productId: String, payType: ApplePayType, source: Int = 0, handle: @escaping IAPcompletionHandle) {
        setupIapDt()
...
extension AppleIAPManager: SKPaymentTransactionObserver {
        ...
        for transaction in transactions {
            ...
            case .purchased:   // 交易成功
                ...
                if transaction.original != nil && createOrderId == nil { // 启动自动续订时，不需要调用服务端验证接口
                    ...
                    // 3. 服务端校验流程
                    chkTxPur(transaction.transactionIdentifier!, self.currentPayType)
                }
                // 移除苹果支付系统缓存
    ...
    ///   - payType: 支付类型
    fileprivate func chkTxPur(_ transactionId: String, _ payType: ApplePayType) {
        guard let receiptStr = reqVfyDt(transactionId, payType) else {
