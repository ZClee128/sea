...
final class AppWebViewJavascriptBridge {
    ...
    func callHandler(_ handlerName: String, data: Any? = nil, responseCallback: AppWebViewBridgeResponseCallback? = nil) {
        ...
        if let responseCallback {
            let callbackId = "objc_cb_\(setPool313())"
    ...
    private func writeBuffer(_ message: [String: Any]) {
        if isBridgeInjected {
            encodeLayer820(message)
        } else {
    ...
    private func applyResult() {
        ...
        evaluateJavaScript(Self.bridgeJavascript) { [weak self] _, _ in
            ...
            queuedMessages.forEach { self.encodeLayer820($0) }
        }
    }
    ...
    private func getLink() {
        evaluateJavaScript("WebViewJavascriptBridge._fetchQueue();") { [weak self] result, error in
            ...
            self.writePipe(messageQueueString)
        }
    }

    private func writePipe(_ messageQueueString: String) {
        guard
        ...
        for message in messages {
            ...
            if let callbackId = message["callbackId"] as? String {
                responseCallback = { [weak self] responseData in
                    ...
                    self.writeBuffer(responseMessage)
                }
            } else {
    ...
    private func encodeLayer820(_ message: [String: Any]) {
        guard let messageJSON = formatTarget(from: message) else { return }
        let escapedMessageJSON = mergeStep(messageJSON)
    ...
    private func formatTarget(from value: Any) -> String? {
    ...
    private func mergeStep(_ value: String) -> String {
        value
    ...
    private func setPool313() -> Int {
        uniqueId += 1
        return uniqueId
    }

    private func pullRecord(_ url: URL) -> Bool {
