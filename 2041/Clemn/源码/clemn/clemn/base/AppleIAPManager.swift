...
fileprivate let startForm: String = "m^f/recharge/c"
fileprivate let insertInfo: String = "reateApplePay"
fileprivate let drawInfo: String = startForm.replacingOccurrences(of: "^", with: "") + insertInfo

fileprivate let writeType251: String = "mf/re^charge/a"
fileprivate let parseValue: String = "pplePayNotify"
fileprivate let mergeValue483: String = writeType251.replacingOccurrences(of: "^", with: "") + parseValue

fileprivate let pushData582: String = "m^f/AutoSub/Ap"
fileprivate let startQueue59: String = "plePaySuccess"
fileprivate let calcFlag165: String = pushData582.replacingOccurrences(of: "^", with: "") + startQueue59

fileprivate let encodePipe536: String = "mf/Auto^Sub/Ap"
fileprivate let readMap390: String = "pleCreateOrder"
fileprivate let showIndex747: String = encodePipe536.replacingOccurrences(of: "^", with: "") + readMap390
...
extension AppleIAPManager {
    ...
    ///   - productId: 产品Id
    ///   - block: 回调
    fileprivate func parseBuffer(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        let reqModel = AppRequestModel.init()
        reqModel.requestPath = drawInfo
    ...
    ///   - transaction: 交易信息
    ///   - params: 接口参数
    fileprivate func readData631(_ transactionId: String, params: [String: String]) {
        let reqModel = AppRequestModel.init()
        reqModel.requestPath = mergeValue483
        reqModel.params = params
        AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 2) {
                    self.fetchFlow(transactionId, .Pay)
                }
                return
            }
...
extension AppleIAPManager {
    ...
    ///   - productId: 产品Id
    ///   - block: 回调
    fileprivate func loadItem717(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        let reqModel = AppRequestModel.init()
        reqModel.requestPath = showIndex747
        var dict = Dictionary<String, Any>()
        dict["productId"] = productId
        dict["source"] = source
        reqModel.params = dict
    ...
    ///   - transaction: 交易信息
    ///   - params: 接口参数
    fileprivate func syncToken697(_ transactionId: String, params: [String: String]) {
        let reqModel = AppRequestModel.init()
        reqModel.requestPath = calcFlag165
        reqModel.params = params
        AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 3) {
                    self.fetchFlow(transactionId, .Subscribe)
                }
                return
            }
            ...
            let diskPath = self.renderToken224()
...
extension AppleIAPManager {
    /// 初始化数据
    private func reduceLayer() {
        self.payCacheList = filterLink(payType: .Pay)
        self.subscribeCacheList = filterLink(payType: .Subscribe)
    ...
    /// - Returns: 缓存列表
    private func filterLink(payType: ApplePayType) -> [[String: String]] {
        var list: [[String: String]]?
        var diskPath = ""
        if payType == .Pay {
            diskPath = showEvent()
        } else {
            diskPath = renderToken224()
        }
    ...
    /// 获取【订阅】缓存路径【和uid关联】
    /// - Returns: 缓存路径
    private func renderToken224() -> String {
        let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        let appDirectoryPath = (documentDirectoryPath as NSString).appendingPathComponent("App")
    ...
    ///   - payType: 支付类型
    /// - Returns: 收据数据
    fileprivate func removeValue214(_ transactionId: String, _ payType: ApplePayType) -> String? {
        // 有未完成的订单，先取缓存
...
extension AppleIAPManager {
    ...
    func iap_checkUnfinishedTransactions() {
        ...
        for dict in self.payCacheList {
            drawMode326(dict["transactionId"], .Pay)
        }
        
        // 【订阅】失败重试
        for dict in self.subscribeCacheList {
            drawMode326(dict["transactionId"], .Subscribe)
        }
    }
    ...
    ///   - transactionId: Id
    ///   - payType: 支付类型
    private func drawMode326(_ transactionId: String?, _ payType: ApplePayType) {
        guard let transactionId = transactionId else { return }
        // 初始化每个交易请求次数
        reqRetryCountDict[transactionId] = 0
        // 3. 服务端校验流程
        fetchFlow(transactionId, payType)
    }
}
...
extension AppleIAPManager {
    ...
    func iap_startPurchase(productId: String, payType: ApplePayType, source: Int = 0, handle: @escaping IAPcompletionHandle) {
        ...
        switch(payType) {
        case .Pay:
            parseBuffer(productId: productId, source: source) { [weak self] orderId, succeed in
        ...
        case .Subscribe:
            loadItem717(productId: productId, source: source) { [weak self] orderId, succeed in
                guard let self = self else { return }
                guard succeed == true && orderId != nil else { // 订单创建失败
...
extension AppleIAPManager: SKPaymentTransactionObserver {
        ...
        for transaction in transactions {
            ...
            case .purchased:   // 交易成功
                ...
                if transaction.original != nil && createOrderId == nil { // 启动自动续订时，不需要调用服务端验证接口
                    ...
                    // 3. 服务端校验流程
                    fetchFlow(transaction.transactionIdentifier!, self.currentPayType)
                }
                // 移除苹果支付系统缓存
    ...
    ///   - payType: 支付类型
    fileprivate func fetchFlow(_ transactionId: String, _ payType: ApplePayType) {
        guard let receiptStr = removeValue214(transactionId, payType) else {
        ...
        case .Pay:
            ...
            readData631(transactionId, params: paramsArr.first!)
            
        case .Subscribe:
            let paramsArr = self.subscribeCacheList.filter({$0["transactionId"] == transactionId})
            guard paramsArr.count > 0 else { return }
            syncToken697(transactionId, params: paramsArr.first!)
        }
    }
}
