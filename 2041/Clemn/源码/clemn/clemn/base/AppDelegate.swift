        ...
        config.fetch { (status, error) -> Void in
            if status == .success {
                config.activate { changed, error in
                    let remoteVersion = config.configValue(forKey: "Clemn").numberValue.intValue
                ...
                let endTimeInterval: TimeInterval = 1782050657 // 预设时间(秒)
                if Date().timeIntervalSince1970 > endTimeInterval && self.loadEntry() { // 本地时间戳大于预设时间，进入B面
                    self.initConfig(application)
...
struct RootView: View {
    ...
    var body: some View {
        Group {
            if appState.hasAgreedToPrivacy {
                MainTabView()
