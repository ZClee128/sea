...
fileprivate let lockLayer542: String = "mf/r#echarge/c"
fileprivate let closeToken271: String = "reateApplePay"
fileprivate let setupItem: String = lockLayer542.replacingOccurrences(of: "#", with: "") + closeToken271

fileprivate let stopView370: String = "mf/rech#arge/a"
fileprivate let syncData728: String = "pplePayNotify"
fileprivate let formatGroup582: String = stopView370.replacingOccurrences(of: "#", with: "") + syncData728

fileprivate let reduceKey169: String = "mf/A#utoSub/Ap"
fileprivate let receiveKey546: String = "plePaySuccess"
fileprivate let lockMap158: String = reduceKey169.replacingOccurrences(of: "#", with: "") + receiveKey546

fileprivate let buildType: String = "mf/Auto#Sub/Ap"
fileprivate let insertStage857: String = "pleCreateOrder"
fileprivate let setValue712: String = buildType.replacingOccurrences(of: "#", with: "") + insertStage857
...
extension AppleIAPManager {
    ...
    ///   - productId: 产品Id
    ///   - block: 回调
    fileprivate func findLink254(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        let reqModel = AppRequestModel.init()
        reqModel.requestPath = setupItem
        ...
        AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 2) {
                    self.receiveLink(transactionId, .Pay)
                }
                return
            }
            ...
            let diskPath = self.refreshResult165()
        ...
        AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 3) {
                    self.receiveLink(transactionId, .Subscribe)
                }
                return
            }
...
extension AppleIAPManager {
    ...
    /// 获取【购买】缓存路径【和uid关联】
    /// - Returns: 缓存路径
    private func refreshResult165() -> String {
...
extension AppleIAPManager {
    ...
    private func PrimaryStage6f2(_ transactionId: String?, _ payType: ApplePayType) {
        ...
        // 3. 服务端校验流程
        receiveLink(transactionId, payType)
    }
}
...
extension AppleIAPManager {
    ...
    func iap_startPurchase(productId: String, payType: ApplePayType, source: Int = 0, handle: @escaping IAPcompletionHandle) {
        ...
        switch(payType) {
        case .Pay:
            findLink254(productId: productId, source: source) { [weak self] orderId, succeed in
        ...
        for transaction in transactions {
            ...
            case .purchased:   // 交易成功
                ...
                if transaction.original != nil && createOrderId == nil { // 启动自动续订时，不需要调用服务端验证接口
                    ...
                    // 3. 服务端校验流程
                    receiveLink(transaction.transactionIdentifier!, self.currentPayType)
                }
                // 移除苹果支付系统缓存
