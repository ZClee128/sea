...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate {
        ...
        config.fetch { (status, error) -> Void in
            if status == .success {
                config.activate { changed, error in
                    let remoteVersion = config.configValue(forKey: "Monti").numberValue.intValue
    ...
    /// 初始化window
    private func executeStrategy5fb() {
    ...
    private func main() {
        DispatchQueue.main.async {
            // MARK: - AVAudioSession: Enable background audio playback
            // Category .playback allows audio to continue when screen is locked
            // or the app moves to background (requires UIBackgroundModes: audio in Info.plist)
            do {
                ...
                print("[Monti] AVAudioSession setup failed: \(error.localizedDescription)")
            }

            // MARK: - Pre-warm the BackgroundPlaybackManager singleton
            // Ensures notification observers are registered before any view loads
            _ = BackgroundPlaybackManager.shared

            // MARK: - Root Window Setup
            let rootView = MainContainerView()
            self.window?.rootViewController = UIHostingController(rootView: rootView)

            // Force lock UI style to Dark Mode to override system dark mode syncing
