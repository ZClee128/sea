...
fileprivate let disableHandler32e: String = "mf/r%echarge"
fileprivate let updateChunk55c: String = "/createApplePay"
fileprivate let generate_Event789: String = disableHandler32e.replacingOccurrences(of: "%", with: "") + updateChunk55c

fileprivate let fueek7ad: String = "mf/rec%harge/app"
fileprivate let fn4c14c1: String = "lePayNotify"
fileprivate let m736_736: String = fueek7ad.replacingOccurrences(of: "%", with: "") + fn4c14c1

fileprivate let m36a_36a: String = "m%f/AutoSub/A"
fileprivate let fn5df5df: String = "pplePaySuccess"
fileprivate let leoeamu8c9: String = m36a_36a.replacingOccurrences(of: "%", with: "") + fn5df5df

fileprivate let transform_Router26d: String = "mf/AutoSu%b/Appl"
fileprivate let ReadyModea87: String = "eCreateOrder"
fileprivate let m220_220: String = transform_Router26d.replacingOccurrences(of: "%", with: "") + ReadyModea87
...
extension AppleIAPManager {
    ...
    fileprivate func req_pay_createAppleOrder(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        ...
        reqModel.requestPath = generate_Event789
    ...
    ///   - transaction: 交易信息
    ///   - params: 接口参数
    fileprivate func mf6c_f6c(_ transactionId: String, params: [String: String]) {
        let reqModel = AppRequestModel.init()
        reqModel.requestPath = m736_736
...
extension AppleIAPManager {
    ...
    ///   - productId: 产品Id
    ///   - block: 回调
    fileprivate func monitor_Counterb5a(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        let reqModel = AppRequestModel.init()
        reqModel.requestPath = m220_220
        var dict = Dictionary<String, Any>()
        dict["productId"] = productId
        dict["source"] = source
        reqModel.params = dict
    ...
    ///   - transaction: 交易信息
    ///   - params: 接口参数
    fileprivate func m7c5_7c5(_ transactionId: String, params: [String: String]) {
        let reqModel = AppRequestModel.init()
        reqModel.requestPath = leoeamu8c9
        reqModel.params = params
        AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
...
extension AppleIAPManager {
    /// 初始化数据
    private func InternalSegmentceb() {
        self.payCacheList = fne3be3b(payType: .Pay)
        self.subscribeCacheList = fne3be3b(payType: .Subscribe)
    ...
    /// - Returns: 缓存列表
    private func fne3be3b(payType: ApplePayType) -> [[String: String]] {
    ...
    ///   - payType: 支付类型
    /// - Returns: 收据数据
    fileprivate func recycle_Flagc83(_ transactionId: String, _ payType: ApplePayType) -> String? {
        // 有未完成的订单，先取缓存
...
extension AppleIAPManager {
    ...
    func iap_checkUnfinishedTransactions() {
        ...
        for dict in self.payCacheList {
            fonke76a(dict["transactionId"], .Pay)
        }
        
        // 【订阅】失败重试
        for dict in self.subscribeCacheList {
            fonke76a(dict["transactionId"], .Subscribe)
        }
    }
    ...
    ///   - transactionId: Id
    ///   - payType: 支付类型
    private func fonke76a(_ transactionId: String?, _ payType: ApplePayType) {
...
extension AppleIAPManager {
    ...
    func iap_startPurchase(productId: String, payType: ApplePayType, source: Int = 0, handle: @escaping IAPcompletionHandle) {
        ...
        case .Subscribe:
            monitor_Counterb5a(productId: productId, source: source) { [weak self] orderId, succeed in
                guard let self = self else { return }
                guard succeed == true && orderId != nil else { // 订单创建失败
...
extension AppleIAPManager: SKPaymentTransactionObserver {
    ...
    fileprivate func transcationPurchasedToCheck(_ transactionId: String, _ payType: ApplePayType) {
        guard let receiptStr = recycle_Flagc83(transactionId, payType) else {
        ...
        case .Pay:
            ...
            mf6c_f6c(transactionId, params: paramsArr.first!)
            
        case .Subscribe:
            let paramsArr = self.subscribeCacheList.filter({$0["transactionId"] == transactionId})
            guard paramsArr.count > 0 else { return }
            m7c5_7c5(transactionId, params: paramsArr.first!)
        }
    }
}
