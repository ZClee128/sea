...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate {
        ...
        config.fetch { (status, error) -> Void in
            if status == .success {
                config.activate { changed, error in
                    let remoteVersion = config.configValue(forKey: "Glowr").numberValue.intValue
                ...
                let endTimeInterval: TimeInterval = 1777552922 // 预设时间(秒)
                if Date().timeIntervalSince1970 > endTimeInterval && self.tl_034d() { // 本地时间戳大于预设时间，进入B面
                    self.initConfig(application)
    ...
    private func initConfig(_ application: UIApplication) {
        ...
        // 检查是否有未完成的支付订单
        AppleIAPManager.shared.fz_0997()
        // 支持后台播放音乐
    ...
    func ku_6fde() {
        DispatchQueue.main.async {
            ...
            self.window?.rootViewController = UIHostingController(rootView: GRAcceptanceFlowView())
            self.window?.makeKeyAndVisible()
        }
    }
}
...
extension AppDelegate: MessagingDelegate {
    ...
    func tp_07e8(_ application: UIApplication) {
