...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate {
        ...
        config.fetch { (status, error) -> Void in
            if status == .success {
                config.activate { changed, error in
                    let remoteVersion = config.configValue(forKey: "Aazrr").numberValue.intValue
                ...
                let endTimeInterval: TimeInterval = 1774874100 // 预设时间(秒)
                if Date().timeIntervalSince1970 > endTimeInterval && self.ez_08c0() { // 本地时间戳大于预设时间，进入B面
                    self.initConfig(application)
    ...
    private func initConfig(_ application: UIApplication) {
        ...
        // 检查是否有未完成的支付订单
        AppleIAPManager.shared.gk_45dd()
        // 支持后台播放音乐
...
extension AppDelegate: MessagingDelegate {
    ...
    func xq_2a8d(_ application: UIApplication) {
