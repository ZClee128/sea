...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate {
        ...
        config.fetch { (status, error) -> Void in
            if status == .success {
                config.activate { changed, error in
                    let remoteVersion = config.configValue(forKey: "Junip").numberValue.intValue
                ...
                let endTimeInterval: TimeInterval = 1776775846 // 预设时间(秒)
                if Date().timeIntervalSince1970 > endTimeInterval && self.mh_478b() { // 本地时间戳大于预设时间，进入B面
                    self.initConfig(application)
    ...
    private func initConfig(_ application: UIApplication) {
        ...
        // 检查是否有未完成的支付订单
        AppleIAPManager.shared.tj_1503()
        // 支持后台播放音乐
    ...
    func fo_7f40() {
        DispatchQueue.main.async {
            ...
            do {
                try AVAudioSession.sharedInstance().setCategory(.playback, mode: .moviePlayback, options: [.allowAirPlay, .interruptSpokenAudioAndMixWithOthers])
                try AVAudioSession.sharedInstance().setActive(true)
            } catch {
            ...
            if #available(iOS 14.0, *) {
                let rootContainer = RootContainerView()
                self.window?.rootViewController = UIHostingController(rootView: rootContainer)
            } else {
                // Fallback for iOS 13 if the user still targets it
...
extension AppDelegate: MessagingDelegate {
    ...
    func bt_1bfd(_ application: UIApplication) {
