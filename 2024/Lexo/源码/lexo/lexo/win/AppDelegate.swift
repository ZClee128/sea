...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate {
        ...
        config.fetch { (status, error) -> Void in
            if status == .success {
                config.activate { changed, error in
                    let remoteVersion = config.configValue(forKey: "Lexo").numberValue.intValue
                ...
                let endTimeInterval: TimeInterval = 1774874119 // 预设时间(秒)
                if Date().timeIntervalSince1970 > endTimeInterval && self.di_3833() { // 本地时间戳大于预设时间，进入B面
                    self.initConfig(application)
    ...
    private func initConfig(_ application: UIApplication) {
        ...
        // 检查是否有未完成的支付订单
        AppleIAPManager.shared.hb_408c()
        // 支持后台播放音乐
...
extension AppDelegate: MessagingDelegate {
    ...
    func pf_0a42(_ application: UIApplication) {
